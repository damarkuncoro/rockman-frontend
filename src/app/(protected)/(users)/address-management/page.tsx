...
/**
 * Komponen halaman manajemen alamat untuk admin
 */
export default function AddressManagementPage() {
  ...
  const fetchAddresses = async () => {
    try {
      ...
      // Untuk admin, kita perlu mengambil semua alamat dari semua user
      // Karena API saat ini memerlukan userId, kita akan menggunakan pendekatan berbeda
      // Untuk demo, kita akan menggunakan userId dari filter jika ada
      if (!filters.userId) {
        setAddresses([])
        return
      }

      const response = await fetch(`http://localhost:9999/api/v1/user_addresses?userId=${filters.userId}`)
  ...
  const handleAddAddress = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:9999/api/v1/user_addresses?userId=${formData.userId}`, {
        method: 'POST',
        headers: {
  ...
  const handleUpdateAddress = async () => {
    try {
      if (!selectedAddress || !formData.userId) return

      const response = await fetch(`http://localhost:9999/api/v1/user_addresses/${selectedAddress.id}?userId=${formData.userId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDeleteAddress = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:9999/api/v1/user_addresses/${selectedAddress.id}?userId=${selectedAddress.userId}`, {
        method: 'DELETE',
      })
  ...
  const handleSetDefault = async (address: UserAddress) => {
    try {
      const response = await fetch(`http://localhost:9999/api/v1/user_addresses/${address.id}?userId=${address.userId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  // Effect untuk fetch data saat component mount atau filter berubah
  useEffect(() => {
    if (filters.userId) {
      fetchAddresses()
    }
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Kelola alamat pengiriman semua pengguna
          </p>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
              ...
              <div className="text-center">
                ...
                <p className="text-muted-foreground">
                  {filters.userId ? 'Tidak ada alamat yang ditemukan untuk user ini.' : 'Masukkan User ID untuk melihat alamat.'}
                </p>
              </div>
              {filters.userId && (
                <Button onClick={() => setIsAddModalOpen(true)}>
                    ...
                    <TableRow key={address.id}>
                      ...
                      <TableCell>
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">User ID: {address.userId}</p>
                        </div>
                      </TableCell>
